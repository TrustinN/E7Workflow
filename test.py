...
class ExprNode:
    ...
    def setOperator(self, op, value=None):
        ...
        if op in (LITERAL, VARIABLE, FUNC, ASSIGNMENT) and value:
            self.value = value
        else:
            self.value = ""
    ...
    def evalChild(self, i, ctx):
        return self.children[i].evaluate(ctx)
    ...
    def evaluate(self, context):
        ...
        if self.op == VARIABLE:
            return ctx[self.value], context

        if self.op == LITERAL:
            return parseLiteral(self.value), context

        if self.op == "AND":
            values = []

            for child in self.children:
                value, ctx = child.evaluate(ctx)
                values.append(value)

            return all(values), context
        ...
        if self.op == "OR":
            ...
            return any(values), context

        if self.op == "NOT":
            value, ctx = self.evalChild(0, ctx)
            return not value, context

        if self.op in {"==", "<", ">", "<=", ">="}:
            left, ctx = self.evalChild(0, ctx)
            right, ctx = self.evalChild(1, ctx)
            ...
            return ops[self.op](left, right), context
...
class ExprModel(QAbstractItemModel):
    ...
    def data(self, index: QModelIndex, role=Qt.DisplayRole):
        ...
        if role == Qt.ItemDataRole.DisplayRole:
            ...
            if item.op == ASSIGNMENT:
                padding = " " if item.value else ""
                return f"{item.value}{padding}{applyTypeHint(item.op)}"
...
class ExprEditor(QWidget):
    ...
    def onOperatorChanged(self, operator):
        self.line.setVisible(operator in (VARIABLE, LITERAL, FUNC, ASSIGNMENT))
        self.addBtn.setVisible(operator not in (VARIABLE, LITERAL, NOOP))
        self.subBtn.setVisible(operator not in (VARIABLE, LITERAL, NOOP))
